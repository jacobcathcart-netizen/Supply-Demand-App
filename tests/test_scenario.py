...
class TestAssembleOutput:
    def test_gap_is_supply_minus_demand(self):
        ...
        result = _assemble_output(alloc, demand, demand)
        row = result.iloc[0]
        ...
        demand = pd.DataFrame(
            {
                "CCRID": ["P001", "P002", "P003"],
                "MONTH_NUMBER": [1, 1, 1],
                "DEMAND_HOURS": [400, 300, 100],
            }
        )
        orig_totals = pd.Series(
            [0.8],
            index=pd.MultiIndex.from_tuples(
                [("East", 1)], names=["REGION", "MONTH_NUMBER"]
            ),
            name="_ORIG_ALLOC_TOTAL",
        )
        result = _recalculate_weights_from_demand(weights, demand, orig_totals)
        assert result.groupby(["REGION", "MONTH_NUMBER"])[
            "ALLOCATION"
        ].sum().iloc[0] == pytest.approx(0.8)
...
class TestExclusionPreservesBaseline:
    ...
    def _run(self, **extra):
        with (
            ...
            return run_scenario(**{**self._common_kwargs, **extra})
    ...
    def test_partial_date_exclusion(self):
        ...
        p002_m1 = p002[p002["DATE"].dt.month == 1].iloc[0]
        assert p002_m1["SCENARIO_SUPPLY"] > 0
        assert p002_m1["SCENARIO_DEMAND"] > 0

        p002_m2 = p002[p002["DATE"].dt.month == 2].iloc[0]
        assert p002_m2["SCENARIO_SUPPLY"] == 0
...
class TestCustomProjectReceivesSupply:
    ...
    def test_custom_project_has_project_name(self):
        ...
        custom_rows = result[result["CCRID"] == "CUSTOM_001"]
        assert not custom_rows["PROJECT_NAME"].isna().any()
        assert custom_rows["PROJECT_NAME"].iloc[0] == "NewProject"
